...
const Appointments: React.FC<AppointmentsProps> = ({ appointments, patients, onAddAppointment }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const endTime = new Date(dateObj.getTime() + 30 * 60000); // 30 min duration default
  ...
  const handleUpdate = (e: React.FormEvent) => {
    ...
    // Local update simulation (would call API update in real app if prop provided)
    alert("Update functionality would trigger API call here.");
    setIsEditMode(false);
  ...
  const handleRowClick = (appt: Appointment) => {
    ...
    setIsEditMode(false);
    // Handle date string coming from backend or mock
    // Backend usually sends ISO strings for date/startAt
    const dateVal = (appt as any).startAt || appt.date;
                    ...
                    {filteredAppointments.map((appt) => {
                      ...
                      return (
                        ...
                        >
                            ...
                            {new Date(dateVal).toLocaleDateString()} <br/>
                            <span className="text-gray-500 font-normal">{new Date(dateVal).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                            </td>
          ...
          {!isEditMode ? (
            <div className="space-y-6">
                ...
                <div>
                    ...
                    <p className="font-medium text-gray-900">{new Date((selectedAppointment as any).startAt || selectedAppointment.date).toLocaleDateString()}</p>
                    <p className="text-sm text-gray-600">{new Date((selectedAppointment as any).startAt || selectedAppointment.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                </div>
                <div>
